module Game exposing (GameState, step, enqueueEvent, fpsMillis, defaultGameState)

import Engine.Environment exposing (Environment, environ)

import Engine.Vector exposing (vec2)
import Snake exposing (Snake, Block, snake)
...
checkGameOver state =
  if state.status == GameOver then defaultGameState else state
...
moveSnake state = { state | snake =  Snake.move state.snake }
